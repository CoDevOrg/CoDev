...
import { 
  User, Bot, Plug, Key, Sliders, 
  Settings, CreditCard, Users, ShieldAlert, ArrowLeft 
} from 'lucide-react';
...
const personalNav = [
  ...
  { name: 'Preferences', href: '/settings/personal/preferences', icon: Sliders },
];
...
export function SettingsSidebar() {
  ...
  return (
    <aside className="w-64 border-r border-zinc-800 bg-zinc-950 p-4 min-h-screen text-zinc-300">
      ...
      <div className="mb-6">
        <h3 className="px-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Personal</h3>
      ...
      <div>
        <h3 className="px-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Organization</h3>
        <nav className="space-y-1">
          {orgNav.map((item) => {
            const isActive = pathname === item.href;
            return (
...
export async function checkOrgSettingsAccess(userId: string, orgId: string, action: 'read' | 'write') {
  ...
  if (!allowed) {
    throw new Error('Unauthorized: You do not have permission to modify organization settings.');
  }
}
