...
describe("LandingWorkspaceDemo", () => {
  ...
  it("lets a visitor inspect every deterministic phase", () => {
    ...
    expect(screen.getByLabelText("Merging worktrees into main")).toBeVisible();
  ...
  it("gives every agent a deterministic typing turn", () => {
    ...
    expect(activeFileForElapsed(8_500).agent.name).toBe("Claude");
    expect(activeFileForElapsed(11_000).agent.name).toBe("Cursor");
  });
