...
export const POST = withWorkspace<{ workspaceId: string; sessionId: string }>(
  ...
  async ({ request, user, workspaceId, params: { sessionId } }) => {
    ...
    try {
      ...
      const turnsToCopy = selectTurnsThroughReply(
        sourceTurns,
        input.fromTurnId,
      );
      ...
      const reservation = await getDatabase().transaction(
        async (transaction) => {
          ...
          if (!worktree)
            throw new Error("Could not reserve a branched worktree.");
          ...
          if (!session)
            throw new Error("Could not create the branched session.");
