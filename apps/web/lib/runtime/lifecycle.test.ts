...
vi.mock("../workspaces/audit", () => ({ appendWorkspaceEvent: vi.fn() }));
vi.mock("../platform/observability", () => ({ logEvent: vi.fn() }));
vi.mock("../providers/claude-connection-session", () => ({
  reapExpiredClaudeConnectionSessions: vi
    .fn()
    .mockResolvedValue({ cleaned: 0, failures: 0 }),
}));
vi.mock("../providers/claude-connection-runner", () => ({
...
vi.mock("../workspaces/workspaces", () => ({ markWorkspaceStopped: vi.fn() }));
...
describe("reconcileLifecycle", () => {
  ...
  it("reaps expired Claude connection runners", async () => {
    const { reapExpiredClaudeConnectionSessions } =
      await import("../providers/claude-connection-session");
