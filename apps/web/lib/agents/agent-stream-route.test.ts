...
vi.mock("@/lib/providers/ai-model", () => ({
...
vi.mock("@/lib/agents/agent-rate-limit", async (importOriginal) => ({
  // The real class, so the test covers the 429 body it builds for itself.
  AgentPromptRateLimitError: (
    await importOriginal<typeof import("@/lib/agents/agent-rate-limit")>()
  ).AgentPromptRateLimitError,
...
import { AgentPromptRateLimitError } from "@/lib/agents/agent-rate-limit";
