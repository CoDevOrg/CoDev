...
vi.mock("@/lib/agents/agent-review", () => ({
...
vi.mock("@/lib/runtime/orchestrator", () => ({
...
import { DELETE } from "@/app/api/workspaces/[workspaceId]/agents/[sessionId]/stop/route";

describe("DELETE agent stop", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getApiUser.mockResolvedValue({ id: "user-1" });
    mocks.ensureWorkspaceRuntimeReady.mockResolvedValue(undefined);
    mocks.discardAgentWorktree.mockResolvedValue({ status: "stopped" });
  });

  it("uses the co-steer lifecycle for Mission Control stop", async () => {
