...
2. **Orca IDE session** — one `orca serve` process per workspace, embedded in
   `apps/web` as a same-origin iframe (`components/workspace/orca-workspace.tsx`). There
   is **no parallel `/ide` route tree** in `apps/web` — the iframe talks to the
   parent page over `postMessage`
   (`packages/ide` → `codev-bridge.ts`; parent → `components/workspace/codev-parent-bridge.ts`),
...
readiness (see `apps/web/lib/runtime/orca-pairing.ts` and
...
The runtime runs on **Azure** only (`infra/azure/`, with the cloud-neutral host
bootstrap and Orca build in `infra/runtime/`). There is no cloud-selection
branch; do not describe the runtime as AWS-hosted or reintroduce one.
