...
describe('agent session resume metadata', () => {
  ...
  it('captures the Cursor conversation id from its hook payload', () => {
    expect(
      extractAgentProviderSession('cursor', { conversation_id: 'c3395695-abcd' })
    ).toEqual({ key: 'session_id', id: 'c3395695-abcd' })
    expect(extractAgentProviderSession('cursor', { session_id: 'cursor-session' })).toEqual({
      key: 'session_id',
      id: 'cursor-session'
    })
    expect(extractAgentProviderSession('cursor', {})).toBeNull()
