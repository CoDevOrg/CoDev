...
function codexEffort(maximum: 'high' | 'xhigh' | 'max' | 'ultra'): CatalogOption {
  const maximumIndex = CODEX_EFFORT_CHOICES.findIndex((choice) => choice.value === maximum)
  return {
    id: 'effort',
    ...
    kind: {
      type: 'select',
      choices: CODEX_EFFORT_CHOICES.slice(0, maximumIndex + 1),
      defaultValue: 'medium'
    },
    apply: {
