...
function createCliApi(): NonNullable<Partial<PreloadApi>['cli']> {
  const status = {
    platform: getBrowserPlatform(),
    commandName: getBrowserPlatform() === 'linux' ? 'codev' : 'orca',
    commandPath: null,
    pathDirectory: null,
...
function getStoredSettings(): GlobalSettings {
  ...
  if (isCodevEmbedded()) {
    ...
    // inside CoDev: every agent in the workspace is already on screen in the
    // right sidebar's always-present "Live agents" tab. Force both experimental
    // surfaces off so a member whose account has them enabled does not see two
    // competing agent lists (this also suppresses the AgentDashboardSidebarHost
    // popout, which is gated on the same flag).
    merged.experimentalActivity = false
    merged.experimentalAgentDashboardPopout = false
  }
  return merged
}
