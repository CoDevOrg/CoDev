...
describe('web pairing input', () => {
  ...
  it('parses query-form pairing URLs', () => {
    expect(parseWebPairingInput(`codev://pair?code=${encodeOffer()}`)).toEqual(offer)
  })

  it('still parses legacy hash-form pairing URLs', () => {
    expect(parseWebPairingInput(`codev://pair#${encodeOffer()}`)).toEqual(offer)
  })

  it('preserves optional device scope metadata', () => {
    expect(parseWebPairingInput(`codev://pair?code=${encodeOffer({ scope: 'mobile' })}`)).toEqual({
      ...offer,
      scope: 'mobile'
    })
  })
  ...
  it('treats invalid device scope metadata as unknown', () => {
    expect(parseWebPairingInput(`codev://pair?code=${encodeOffer({ scope: 'admin' })}`)).toEqual(
      offer
    )
  })

  it('rejects orca URLs outside the exact pairing route', () => {
    expect(parseWebPairingInput(`codev://pairing?code=${encodeOffer()}`)).toBeNull()
    expect(parseWebPairingInput(`codev://pair-extra?code=${encodeOffer()}`)).toBeNull()
  })

  it('auto-saves scoped runtime offers during web startup', () => {
    const input = `codev://pair?code=${encodeOffer({ scope: 'runtime' })}`
    expect(
  ...
  it('shows the connect screen for mobile-scope and legacy unknown-scope offers', () => {
    const mobileInput = `codev://pair?code=${encodeOffer({ scope: 'mobile' })}`
    const legacyInput = `codev://pair?code=${encodeOffer()}`
