...
describe('composer drop upload result', () => {
  it('separates imported files and folders while counting skipped or failed paths', () => {
    const results: ComposerDropUploadImportResult[] = [
      { status: 'imported', kind: 'file', destPath: '/repo/.codev/drops/file.txt' },
      { status: 'imported', kind: 'directory', destPath: '/repo/.codev/drops/folder' },
      { status: 'skipped' },
      { status: 'failed' }
    ]

    expect(collectComposerDropUploadResult(results)).toEqual({
      filePaths: ['/repo/.codev/drops/file.txt'],
      folderPaths: ['/repo/.codev/drops/folder'],
      skippedOrFailed: 2
    })
  })
