...
export function decideInitialAgentTabViewMode(args: {
  ...
  // which can beat the async settings read — and the resulting `undefined`
  // was sent to the paired host as `viewMode: 'terminal'`, so the tab looked
  // right locally and came back a raw TUI on the next reload.
  const codevForcesChat = isCodevEmbedded()
  if (
    !codevForcesChat &&
