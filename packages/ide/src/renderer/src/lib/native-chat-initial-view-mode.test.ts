...
describe('CoDev-embedded clients', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })

  it('opens a supported agent in chat even before settings have loaded', () => {
    // launchCodevDefaultChatTab fires right after project activation and can
    // beat the async settings read; returning undefined here sent the paired
    // host viewMode 'terminal', so the tab came back a raw TUI on reload.
    vi.stubGlobal('window', { __CODEV_EMBEDDED__: true, location: { hash: '' } })
    expect(
      decideInitialAgentTabViewMode({ agent: 'claude' })
    ).toBe('chat')
  })
