...
import { CodevWorkspaceInvitesSection } from './CodevWorkspaceInvitesSection'
import { CodevWorkspaceMemberRolesSection } from './CodevWorkspaceMemberRolesSection'
import { isCodevSettingsOnly } from './codev-personal-settings'
...
export function GeneralWorkspaceSettingsSection({
  settings,
  updateSettings
}: GeneralWorkspaceSettingsSectionProps): React.JSX.Element | null {
  // CoDev personal surface: there is no workspace in context, so nothing in
  // this pane (invites, member roles, directory, delete confirmations) has
  // anywhere to apply. The personal settings that do belong here (profile,
  // provider connections) live in their own dedicated tabs instead.
  if (isCodevSettingsOnly()) {
    return null
  }
  ...
  return (
    <section key="workspace" className="space-y-4">
      ...
      <CodevWorkspaceInvitesSection />
      <CodevWorkspaceMemberRolesSection />
