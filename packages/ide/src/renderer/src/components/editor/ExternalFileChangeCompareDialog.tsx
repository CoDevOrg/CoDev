...
import {
  CODEV_EXTERNAL_FILE_CONFLICT_COMPARE,
...
export function ExternalFileChangeCompareDialog({
  ...
  onReload: () => void
  onKeepEdits: () => void
  onMerge?: (mergedContents: string) => void
}): React.JSX.Element {
  const [diskState, setDiskState] = useState<DiskReadState>({ kind: 'loading' })
  const [mergedContents, setMergedContents] = useState(currentContent)
  ...
  useEffect(() => {
    ...
    setMergedContents(currentContent)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[80vh] w-[90vw] max-w-5xl flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl">
        <DialogHeader className="border-b border-border/60 p-4">
          <DialogTitle>
            {translate(
              'auto.components.editor.ExternalFileChangeCompareDialog.codevPreserveBothTitle',
              CODEV_EXTERNAL_FILE_CONFLICT_LABEL
            )}
          </DialogTitle>
          <DialogDescription>
            {translate(
              'auto.components.editor.ExternalFileChangeCompareDialog.codevPreserveBoth',
              CODEV_EXTERNAL_FILE_CONFLICT_COMPARE
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex-col items-stretch gap-3 border-t border-border/60 p-4 sm:flex-col">
          {diskState.kind === 'ready' && (
            <label className="codev-conflict-merge">
              Merge manually
              <textarea
                value={mergedContents}
                onChange={(event) => setMergedContents(event.target.value)}
                aria-label="Merged file contents"
              />
            </label>
          )}
