...
import {
  ...
  type CodevExternalFileConflict,
...
export function ExternalFileChangeBanner({
  ...
  const [codevConflict, setCodevConflict] = useState<CodevExternalFileConflict | null>(null)

  useEffect(() => {
    let cancelled = false
    void reportCodevExternalFileConflict({
      path: file.relativePath,
      collaborativeContents: currentContent
    }).then((conflict) => {
      if (!cancelled) {
        setCodevConflict(conflict)
      }
    })
    return () => {
      cancelled = true
    }
  }, [currentContent, file.relativePath])
      ...
      if (codevConflict) {
        try {
          await resolveCodevExternalFileConflict({
            conflict: codevConflict,
            strategy
          })
        } catch {
          // Native recovery still applies if durable resolution is unavailable.
        }
      }
      apply()
    })()
  ...
  const handleReload = (): void => {
    ...
    resolveThen('filesystem', () => reloadTabContentFromDisk(file, reloadContent))
  }
  const handleKeepEdits = (): void => {
    trackExternalChangeConflictAction(file, 'keep')
    resolveThen('collaboration', () => keepTabEditsOverExternalChange(file))
  }
  const handleMerge = (mergedContents: string): void => {
    void (async () => {
      if (codevConflict) {
        try {
          await resolveCodevExternalFileConflict({
          ...
          // Fall through to keep the merged buffer locally.
        }
      }
      const state = useAppStore.getState()
      state.setEditorDraft(file.id, mergedContents)
      state.markFileDirty(file.id, false)
      state.setExternalMutation(file.id, null)
      reloadContent(file)
    })()
  ...
  return (
    ...
    <div
      role="alert"
      aria-label={CODEV_EXTERNAL_FILE_CONFLICT_LABEL}
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="min-w-0 font-medium text-foreground">
            {translate(
              'auto.components.editor.ExternalFileChangeBanner.codevPreserveBoth',
              CODEV_EXTERNAL_FILE_CONFLICT_BANNER
            )}
          </span>
        </div>
