...
vi.mock('@/web/codev-proposal-discard', () => ({
  getCodevProposalWorktreeId: (path: string, comment = '') =>
    path.includes('/.git/codev-agent-worktrees/')
      ? path.split('/').at(-1)
      : comment.startsWith('codev-proposal:')
        ? comment.slice('codev-proposal:'.length)
        : null,
  requestCodevProposalDiscard: vi.fn()
}))

import { runWorktreeDeletesInParallel } from './delete-worktree-flow'
import { requestCodevProposalDiscard } from '@/web/codev-proposal-discard'
...
describe('DeleteWorktreeDialog lineage copy', () => {
  beforeEach(() => {
    ...
    vi.mocked(requestCodevProposalDiscard).mockReset()
  })
  ...
  it('routes a managed CoDev proposal through audited discard then native worktree removal', async () => {
    ...
    mocks.state.modalData = { worktreeId: workspace.id, onDeleted }
    mocks.state.allWorktrees.mockReturnValue([workspace])
    vi.mocked(runWorktreeDeletesInParallel).mockResolvedValue([workspace.id])
    vi.mocked(requestCodevProposalDiscard).mockResolvedValue({
  ...
  it('routes a comment-tagged native CoDev proposal through audited discard', async () => {
    const worktreeId = 'c1f9fe13-6881-44a6-adbd-96bc5a946afa'
    const workspace = {
      ...makeWorktree('Managed proposal', '/srv/codev/workspaces/workspace-agent'),
      comment: `codev-proposal:${worktreeId}`
    }
    const onDeleted = vi.fn()
    mocks.state.modalData = { worktreeId: workspace.id, onDeleted }
    mocks.state.allWorktrees.mockReturnValue([workspace])
    ...
    expect(requestCodevProposalDiscard).toHaveBeenCalledWith(workspace.path, {
