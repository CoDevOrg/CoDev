...
/**
 * Re-runs the setup-script prompt inspection when a shared `codev.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: codev.yaml is edited on disk or the hook runs in a terminal outside React
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable codev.yaml into a readable one,
