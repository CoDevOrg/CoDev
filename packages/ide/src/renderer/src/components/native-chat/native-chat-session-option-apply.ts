...
async function applySetOption(
  ...
  if (ctx.mode === 'live' && apply.midSession?.kind === 'agent-picker') {
    if (!ctx.applyAgentPickerChoice) {
      throw new Error('This option must be changed in the agent picker.')
    }
    await ctx.applyAgentPickerChoice({ optionId: id, value, modelId: previousModelId })
    ctx.setTrackedValue(id, value, 'applied')
    return finish(ctx, { modelId: previousModelId, optionId: id, value })
  }
