...
function DescriptorMenuRows(props: {
  ...
  // Some agent pickers can be driven safely behind the native surface. Those
  // expose their real choices here so the user never has to leave chat.
  if (descriptor.action?.type === 'agent-picker' && descriptor.id !== 'effort') {
    return (
      <DropdownMenuItem disabled={!descriptor.settable || pending} onSelect={() => invokeAction()}>
        {translate(
