...
describe('NativeChatSessionOptionPickers', () => {
  ...
  it('keeps reasoning effort choices in native chat when the picker is driven behind it', async () => {
    ...
    expect(screen.queryByText('Choose in agent picker…')).toBeNull()
    screen.getByRole('radio', { name: 'High' }).click()
    await waitFor(() => expect(setOption).toHaveBeenCalledWith('effort', 'high'))
  })
