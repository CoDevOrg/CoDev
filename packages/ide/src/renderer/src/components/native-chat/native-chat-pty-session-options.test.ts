...
describe('native chat PTY session options', () => {
  ...
  it('applies a Codex effort choice without opening the terminal picker surface', async () => {
    seedNativeChatAppliedSessionOptions('pty-codex', 'codex', {
      model: 'gpt-5.6-terra',
      effort: 'medium'
    })
    ...
    expect(surface.getSnapshot()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: 'effort',
          kind: expect.objectContaining({ currentValue: 'high' })
        })
      ])
    )
  })
