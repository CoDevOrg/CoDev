...
function MessageRow({
  ...
  /** This message's own span, used for the tool run it contains. */
  durationMs?: number | null
  /** Set on the last message of a turn: how long the whole answer took. */
  ...
  return (
    ...
    >
      {isReasoning && formatTurnDuration(durationMs) ? (
        <p className="mb-1 font-mono text-[10.5px] not-italic text-muted-foreground/80">
          {translate('components.native-chat.thoughtFor', 'Thought for {{value0}}', {
            value0: formatTurnDuration(durationMs) ?? ''
          })}
        </p>
      ) : null}
      {showControls ? (
      ...
      {tools.length > 0 ? (
        <NativeChatToolRun
          ...
          durationLabel={formatTurnDuration(durationMs)}
        />
      ) : null}
      {formatTurnDuration(turnTotalMs) ? (
        <p className="mt-2 font-mono text-[10.5px] text-muted-foreground/70">
          {translate('components.native-chat.respondedIn', 'Responded in {{value0}}', {
            value0: formatTurnDuration(turnTotalMs) ?? ''
          })}
        </p>
      ) : null}
    </div>
...
export function NativeChatMessageList({
  ...
  // Only tick while the agent is working: the in-progress turn's elapsed time
  // is the one number that moves, and re-rendering a settled transcript once a
  // second buys nothing.
  ...
  const turnDurations = useMemo(
    () => nativeChatTurnDurations(session.messages, now),
    [session.messages, now]
  )
  const turnTotals = useMemo(
    () => nativeChatTurnTotals(session.messages, now, isWorking),
    [session.messages, now, isWorking]
  )
  ...
  return (
    <div className="relative min-h-0 flex-1">
      ...
      >
        <div
          ref={contentRef}
          // Stable hook for CoDev's chat theming (orca-theme-overrides.css);
          // authored here rather than relying on compiled Tailwind names.
        ...
        >
          ...
          {messages.map((message, index) => (
            <MessageRow
              key={message.id}
              message={message}
              durationMs={turnDurations[index] ?? null}
              turnTotalMs={turnTotals[index] ?? null}
              expandSignal={expandSignal}
