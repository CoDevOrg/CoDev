...
function NativeChatResolvedView({
  ...
  const [terminalFallbackMessages, setTerminalFallbackMessages] = useState(() =>
    agent === 'codex' ? codexTerminalScreenToMessages(readTerminalScreen?.() ?? null) : []
  )
  useEffect(() => {
    if (agent !== 'codex' || !readTerminalScreen) {
      setTerminalFallbackMessages([])
      return
    }
    const refresh = (): void => {
      setTerminalFallbackMessages(codexTerminalScreenToMessages(readTerminalScreen()))
    }
    refresh()
  ...
  }, [session, terminalFallbackMessages])
  ...
  const launchDraftSignal = useNativeChatLaunchDraftSignal({
    terminalTabId,
    agent,
    messages: sessionWithTerminalFallback.messages,
    transcriptLoading: sessionWithTerminalFallback.readPhase === 'loading'
  })
  ...
  const liveWorking = sessionWithTerminalFallback.status === 'working'
  ...
  useEffect(() => {
    setPending((prev) =>
      writePendingSendCache(
        pendingScope,
        prunePendingSends(prev, sessionWithTerminalFallback.messages)
      )
    )
  }, [sessionWithTerminalFallback.messages, pendingScope])
  useEffect(() => {
    if (
      !paneLaunchPrompt ||
      !shouldPruneLaunchPrompt(paneLaunchPrompt, sessionWithTerminalFallback.messages)
    ) {
      return
    }
    ...
    paneLaunchPrompt,
    sessionWithTerminalFallback.messages,
    terminalTabId
  ])
  const onOptimisticSend = useCallback(
    (text: string, imagePaths?: string[]) => {
      ...
      const boundary = sessionWithTerminalFallback.messages.at(-1)
    ...
    [pendingScope, sessionWithTerminalFallback.messages]
  )
  ...
  const launchPromptMessage = useMemo(
    () => launchPromptAsMessage(paneLaunchPrompt, sessionWithTerminalFallback.messages),
    [paneLaunchPrompt, sessionWithTerminalFallback.messages]
  )
  const sessionWithLaunchPrompt = useMemo<typeof session>(() => {
    if (!launchPromptMessage) {
      return sessionWithTerminalFallback
    }
    return {
      ...sessionWithTerminalFallback,
      messages: [...sessionWithTerminalFallback.messages, launchPromptMessage]
    }
  }, [launchPromptMessage, sessionWithTerminalFallback])
