...
describe('useNativeChatSessionOptions pre-PTY draft surface', () => {
  beforeEach(() => {
    discoverModels.mockReset()
    discoverModels.mockResolvedValue(null)
    Reflect.deleteProperty(window, '__CODEV_EMBEDDED__')
  })

  it('has no surface before the PTY exists in stock Orca', () => {
    const { result } = renderHook(() =>
      useNativeChatSessionOptions({
        agent: 'claude',
        terminalTabId: 'tab-draft',
        targetPtyId: null,
  ...
  it('exposes the draft model picker before the PTY mirrors when CoDev-embedded', () => {
