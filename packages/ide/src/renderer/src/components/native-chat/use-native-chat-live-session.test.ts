...
describe('useNativeChatLiveSession — notFound retry (#8401)', () => {
  ...
  it('treats a still-missing transcript as an empty conversation once the ~60s window is exhausted, never an error', async () => {
    vi.useFakeTimers()
    const transport = getMockTransport('env-1', { autoSnapshot: false })
    ...
    // A brand-new session whose transcript has not flushed is empty, not broken:
    // the pane must never show "Could not load conversation" for it.
    expect(latest?.status).not.toBe('error')
    expect(latest?.error).toBeUndefined()
    expect(latest?.messages).toEqual([])
  })

  it('keeps polling slowly after the window and upgrades to the transcript when it finally flushes', async () => {
    vi.useFakeTimers()
    const transport = getMockTransport('env-1', { autoSnapshot: false })
    transport.readSession
      .mockResolvedValue({ error: 'No transcript found', notFound: true })
      .mockResolvedValueOnce({ error: 'No transcript found', notFound: true })
    ...
    transport.readSession.mockResolvedValue({ messages: [assistant('late-1', 'here now')] })
    await act(async () => {
      await vi.advanceTimersByTimeAsync(11_000)
    })

    expect(latest?.status).not.toBe('error')
    expect(latest?.messages.map((m) => m.id)).toContain('late-1')
  })
