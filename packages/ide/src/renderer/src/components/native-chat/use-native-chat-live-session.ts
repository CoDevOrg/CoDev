...
export function useNativeChatLiveSession(
  ...
  useEffect(() => {
    ...
    let notFoundSurfacedAsEmpty = false
        ...
        .then((result) => {
          ...
          if (result && 'error' in result) {
            if (result.notFound) {
              ...
              // empty conversation, not a failure. Show the empty state — never
              // "Could not load conversation" — and keep polling slowly so a
              // late first flush still upgrades the pane.
              if (!notFoundSurfacedAsEmpty) {
                notFoundSurfacedAsEmpty = true
                setRead({ phase: 'ready', messages: [] })
              }
