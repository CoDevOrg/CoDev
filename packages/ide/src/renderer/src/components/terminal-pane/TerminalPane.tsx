...
function TerminalPane(
  ...
  }, [isChatViewMode, setTabViewMode, terminalTab?.launchAgent, unifiedTabId])
  ...
  const isChatEligibleForLeaf = useCallback(
    (leafId: string | null): boolean => {
      // CoDev: the workspace agent tab is a chat surface by definition. During
      // cold start the PTY is still booting and no agent hook has landed, so
      // the generic checks below would report the leaf ineligible and the
      // route resolver would demote the tab to the raw agent TUI. While the tab
      // is already in chat mode, keep it eligible so it never drops out.
      if (isCodevEmbedded() && isChatViewMode) {
        return true
      }
  ...
  const applyNativeChatLeafRoute = useCallback(
    (route: NativeChatLeafRoute): void => {
      // CoDev: never auto-demote the workspace agent tab to the raw agent TUI.
      // A cold-start eligibility miss or a spurious mid-boot "agent exited"
      // signal must not strip the chat surface — that surface is the whole
      // point of the embed, and the composer re-launches the agent on the next
      // send. Keep chat mode and anchor it to the live pane; normal routing
      // resumes once the agent emits its first event.
      if (isCodevEmbedded() && route.exitChat) {
        const liveLeaf =
          route.chatLeafId ?? managerRef.current?.getActivePane()?.leafId ?? chatLeafId
        if (liveLeaf !== chatLeafId) {
          setChatLeafId(liveLeaf)
        }
        return
      }
