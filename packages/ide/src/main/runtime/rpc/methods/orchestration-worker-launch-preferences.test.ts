...
describe('orchestration worker launch preferences', () => {
  ...
  it('rejects model-specific effort combinations the catalog disproves', () => {
    // CoDev's catalog raises gpt-5.6-luna's ceiling to `max` (upstream stopped
    // at `high`), so `xhigh` is legitimate here and `ultra` is the first level
    // this model disproves. See agent-session-option-catalog-claude-codex.ts.
    expect(() =>
      resolveWorkerLaunchPreferences({
        agent: 'codex',
        model: 'gpt-5.6-luna',
