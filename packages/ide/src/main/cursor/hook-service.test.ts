  ...
  it('installs Cursor Agent hooks with the documented top-level command schema', () => {
    ...
    for (const eventName of CURSOR_EVENTS) {
      ...
      if (process.platform !== 'win32') {
        expect(definition?.command).toContain(join(homeDir, '.codev'))
      }
      expect(definition?.hooks).toBeUndefined()
    }

    const script = readFileSync(
      join(homeDir, '.codev', 'agent-hooks', CURSOR_SCRIPT_FILE_NAME),
      'utf8'
    )
      ...
      `${JSON.stringify(
        {
          ...
          hooks: {
            beforeSubmitPrompt: [
              ...
              { command: '/old/path/.codev/agent-hooks/cursor-hook.sh' }
            ],
            retiredEvent: [
              { command: '/old/path/.codev/agent-hooks/cursor-hook.sh' },
