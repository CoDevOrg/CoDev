...
describe('fs:importExternalPaths — SSH routing & connection', () => {
  ...
  it('uploads terminal-drop staging marker through provider writes', async () => {
    ...
    await invoke({
      sourcePaths: ['/tmp/dropped/file.txt'],
      destDir: '/home/user/project/.codev/drops',
      connectionId: connId,
      ensureDir: true
    })

    expect(provider.createDir).toHaveBeenCalledWith('/home/user/project/.codev')
    expect(provider.writeFile).toHaveBeenCalledWith(
      '/home/user/project/.codev/.gitignore',
      '*\n!.gitignore\n'
    )
    expect(provider.createDir).toHaveBeenCalledWith('/home/user/project/.codev/drops')
  })
})
