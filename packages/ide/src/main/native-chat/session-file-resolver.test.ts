...
describe('resolveSessionFilePath', () => {
  ...
  it('resolves a Cursor transcript at projects/<slug>/agent-transcripts/<id>/<id>.jsonl', async () => {
    const root = await makeRoot('orca-native-chat-resolve-cursor-')
    const cursorProjectsDir = join(root, 'cursor-projects')
    const id = 'a1b2c3d4-0000-4000-8000-000000000001'
    const sessionDir = join(
      cursorProjectsDir,
      'srv-codev-workspaces-ws-1',
      'agent-transcripts',
      id
    )
    await mkdir(sessionDir, { recursive: true })
    const target = join(sessionDir, `${id}.jsonl`)
    await writeFile(target, '{"type":"turn_ended","status":"success"}\n')

    await expect(
      resolveSessionFilePath('cursor', id, { cursorProjectsDir })
    ).resolves.toBe(target)
  })

  it('ignores a Cursor <id>.jsonl that is not under its id-named agent-transcripts dir', async () => {
    const root = await makeRoot('orca-native-chat-resolve-cursor-decoy-')
    const cursorProjectsDir = join(root, 'cursor-projects')
    const id = 'a1b2c3d4-0000-4000-8000-000000000002'
    const decoy = join(cursorProjectsDir, 'proj', 'notes', `${id}.jsonl`)
    await mkdir(dirname(decoy), { recursive: true })
    await writeFile(decoy, '{}\n')
