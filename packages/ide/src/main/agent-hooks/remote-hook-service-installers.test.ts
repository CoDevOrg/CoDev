...
describe('remote hook service installers', () => {
  ...
  it('installs remote Codex hooks with matching trust entries', async () => {
    ...
    for (const eventName of [
      ...
      expect(command).toContain('/home/dev/.codev/agent-hooks/codex-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    expect(fs.files.get('/home/dev/.codev/agent-hooks/codex-hook.sh')).toContain('#!/bin/sh')
    expect(fs.modes.get('/home/dev/.codev/agent-hooks/codex-hook.sh')).toBe(0o755)
  ...
  it('installs Codex hooks into an explicit redirected CODEX_HOME (WSL managed runtime home)', async () => {
    ...
    expect(hooks.hooks.Stop?.[0]?.hooks?.[0]?.command).toContain(
      '/home/dev/.codev/agent-hooks/codex-hook.sh'
    )
...
`
    ...
    for (const eventName of ['BeforeAgent', 'AfterAgent', 'AfterTool', 'BeforeTool']) {
      ...
      expect(command).toContain('/home/dev/.codev/agent-hooks/gemini-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    for (const eventName of ['PreInvocation', 'PostInvocation', 'Stop']) {
      ...
      expect(command).toContain('/home/dev/.codev/agent-hooks/antigravity-hook.sh')
    ...
    for (const eventName of [
      ...
      expect(definition?.command).toContain('/home/dev/.codev/agent-hooks/cursor-hook.sh')
    ...
    for (const eventName of ['PreToolUse', 'PostToolUse', 'Stop']) {
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.codev/agent-hooks/command-code-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    for (const eventName of [
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.codev/agent-hooks/grok-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    for (const eventName of [
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.codev/agent-hooks/devin-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    expect(devin.fs.files.get('/home/dev/.codev/agent-hooks/devin-hook.sh')).toContain('/hook/devin')
  })
  ...
  it('installs remote Grok config in the explicit guest GROK_HOME', async () => {
    ...
    const script = fs.files.get('/home/dev/.codev/agent-hooks/grok-hook.sh')!
  ...
  it('installs remote Kimi hooks as a managed config.toml block preserving user config', async () => {
    ...
    expect(fs.files.get('/home/dev/.codev/agent-hooks/kimi-hook.sh')).toContain('/hook/kimi')
  })
  ...
  it('does not overwrite malformed remote Devin JSONC', async () => {
    ...
    try {
      ...
      expect(fs.files.get('/home/dev/.codev/agent-hooks/devin-hook.sh')).toBeUndefined()
    } finally {
      warn.mockRestore()
    }
  })
  ...
  it('installs remote Copilot hooks under the user-level hooks directory', async () => {
    ...
    for (const eventName of [
      ...
      expect(definition?.bash).toContain('/home/dev/.codev/agent-hooks/copilot-hook.sh')
    ...
    expect(fs.files.get('/home/dev/.codev/agent-hooks/copilot-hook.sh')).toContain('#!/bin/sh')
    expect(fs.modes.get('/home/dev/.codev/agent-hooks/copilot-hook.sh')).toBe(0o755)
  })
  ...
  it('installs remote Droid hooks into Factory settings.json (issue #7253)', async () => {
    ...
    for (const eventName of [
      ...
      const definition = config.hooks[eventName]?.[0]
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.codev/agent-hooks/droid-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    const script = fs.files.get('/home/dev/.codev/agent-hooks/droid-hook.sh')
    expect(script).toContain('#!/bin/sh')
    expect(script).toContain('/hook/droid')
    expect(fs.modes.get('/home/dev/.codev/agent-hooks/droid-hook.sh')).toBe(0o755)
  })
  ...
  it('does not overwrite a malformed remote Factory settings.json', async () => {
    ...
    expect(fs.files.get('/home/dev/.codev/agent-hooks/droid-hook.sh')).toBeUndefined()
  })
