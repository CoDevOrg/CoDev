...
const CONTENT_YIELD_LINE_COUNT = 200
const CONTENT_YIELD_CHAR_COUNT = 256 * 1024
 ...
 * that already left. */
export function sessionContentLines(
  content: string,
  signal?: AbortSignal
...
async function* cancellableContentLines(
  ...
  for (let index = 0; index <= content.length; index++) {
    ...
    linesSinceYield++
    if (
      linesSinceYield >= CONTENT_YIELD_LINE_COUNT ||
      index - yieldStart >= CONTENT_YIELD_CHAR_COUNT
    ) {
      throwIfAiVaultScanCancelled(signal)
